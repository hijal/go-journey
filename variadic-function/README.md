...
- `nums ...int` -- এই parameter-টি যেকোনো সংখ্যক `int` accept করতে পারে; function-এর ভিতরে এটা একটা `[]int` slice-এ পরিণত হয়.
- `total := 0` -- accumulator, শুরুতে zero value.
- `for _, n := range nums` -- প্রতিটা element-এ loop করে.
...
- `sum(1, 2, 3, 4)` -- চারটা argument দিন; variadic function automatically `[]int{1, 2, 3, 4}` তৈরি করে.
- `result` = `10`
